...
Provides [Django](https://docs.djangoproject.com) model utilities for encouraging direct data access instead of unnecessary object overhead. Implemented through compatible method and operator extensions[^1] to [QuerySets](../reference/QuerySet.qmd) and [Managers](../reference/Manager.qmd).
...
So why not provide syntactic support for the better approach? The [Manager](../reference/Manager.qmd) supports filtering by primary key, since that's so common. The [QuerySet](../reference/QuerySet.qmd) supports column updates.
...
Direct column access has some of the clunkiest syntax: `values_list(..., flat=True)`. [QuerySets](../reference/QuerySet.qmd) override `__getitem__`, as well as comparison operators for simple filters. Both are common syntax in panel data layers.
...
Annotations and updates with `Case` and `When` expressions. See also [bulk_changed and bulk_change](../reference/Manager.qmd#bulk_changed) for efficient bulk operations on primary keys.
