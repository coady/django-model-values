...
class F(models.F, Lookup, metaclass=MetaF):
    ...
    max = partial(models.Max)
    sum = partial(models.Sum)
    mean = partial(models.Avg)
    var = partial(models.Variance)
    std = partial(models.StdDev)
    greatest = partial(functions.Greatest)
    least = partial(functions.Least)
    now = staticmethod(functions.Now)
    cast = partial(functions.Cast)
    extract = partial(functions.Extract)
    trunc = partial(functions.Trunc)
    cume_dist = partial(functions.CumeDist)
    dense_rank = partial(functions.DenseRank)
    first_value = partial(functions.FirstValue)
    lag = partial(functions.Lag)
    last_value = partial(functions.LastValue)
    lead = partial(functions.Lead)
    nth_value = partial(functions.NthValue)
    ntile = staticmethod(functions.Ntile)
    percent_rank = partial(functions.PercentRank)
    rank = partial(functions.Rank)
    row_number = partial(functions.RowNumber)
    strip = partial(functions.Trim)
    lstrip = partial(functions.LTrim)
    rstrip = partial(functions.RTrim)
    repeat = partial(functions.Repeat)
    nullif = partial(functions.NullIf)
    __reversed__ = partial(functions.Reverse)
    __abs__ = partial(functions.Abs)
    __ceil__ = partial(functions.Ceil)
    __floor__ = partial(functions.Floor)
    __mod__ = partial(functions.Mod)
    pi = functions.Pi()
    __pow__ = partial(functions.Power)
    __round__ = partial(functions.Round)
    sha1 = partial(functions.SHA1)
    sha224 = partial(functions.SHA224)
    sha256 = partial(functions.SHA256)
    sha384 = partial(functions.SHA384)
    sha512 = partial(functions.SHA512)
    collate = partial(functions.Collate)
    ...
    if gis:  # pragma: no cover
        ...
        geojson = partial(gis.functions.AsGeoJSON)
        gml = partial(gis.functions.AsGML)
        kml = partial(gis.functions.AsKML)
        svg = partial(gis.functions.AsSVG)
        bounding_circle = partial(gis.functions.BoundingCircle)
        centroid = property(gis.functions.Centroid)
        difference = partial(gis.functions.Difference)
        envelope = property(gis.functions.Envelope)
        geohash = partial(gis.functions.GeoHash)  # __hash__ requires an int
        intersection = partial(gis.functions.Intersection)
        make_valid = partial(gis.functions.MakeValid)
        ...
        reverse = partial(gis.functions.Reverse)
        scale = partial(gis.functions.Scale)
        snap_to_grid = partial(gis.functions.SnapToGrid)
        symmetric_difference = partial(gis.functions.SymDifference)
        transform = partial(gis.functions.Transform)
        translate = partial(gis.functions.Translate)
        union = partial(gis.functions.Union)
        azimuth = partial(gis.functions.Azimuth)
        line_locate_point = partial(gis.functions.LineLocatePoint)
        force_polygon_cw = partial(gis.functions.ForcePolygonCW)

        @partial
        class distance(gis.functions.Distance):
            """Return `Distance` with support for lookups: <, <=, >, >=, within."""

            __lt__ = partial(transform, "distance_lt")
            __le__ = partial(transform, "distance_lte")
            __gt__ = partial(transform, "distance_gt")
            __ge__ = partial(transform, "distance_gte")
